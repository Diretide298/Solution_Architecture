    ...
    for (const screen of list) {
      ...
      screens.set(screen.id, {
        ...
        regions: (screen.layout?.regions ?? []).map((r) => ({
          name: r?.name ?? '',
          ref: r?.ref ?? null,
          // A component has had one of these all along; a region needs one for
          // the same reason. It is what tells a region that is *meant* to be
          // empty — a status strip rendering from `getCurrentShift` rather than
...
export async function buildJourneys(root, contractOperationIds = new Set()) {
  ...
  for (const screen of screens.values()) {
    ...
    for (const region of screen.regions) {
      ...
      // A region with `components: []` and nothing said about why.
      //
      // Empty is not wrong on its own: a `statusStrip` may render from
      // `getCurrentShift` rather than from a component list, and a check that
      // cannot tell that from an unfinished screen gets answered with filler
      // controls nobody wanted. So `notes` on the region silences this — the fix
      // for a correct one is a sentence, and the next reader inherits the answer
      // instead of the doubt.
      //
      // A warning either way: the file parses, nothing downstream dangles, and
      // what this costs is a screen that cannot be built from its own
      // description. The template is named because it is what decides whether
      // the slot was the screen's idea or the template's.
      if (!region.components.length && !region.notes) {
        problems.push({
          severity: 'warning',
          kind: 'screen-empty-region',
          file: screen.file,
          message:
            `Screen ${screen.id} declares region ${region.ref ?? region.name} and puts nothing `
            + `in it${screen.template ? ` on the ${screen.template} template` : ''}, and says `
            + `nothing about why — add components, or a note saying what fills it`,
        });
      }
