...
//
// "Every copy" used to mean every .xlsx in these folders, on the assumption
// that any spreadsheet in the delivery was a build of this one. The 24 August
// drop ended that: it added handoff/TICVAI_Services_and_Data_Segregation.xlsx,
// a different workbook on a different subject, stamped the same minute as the
// schema reference. Newest-by-clock took it, and since none of the sheets read
// below are in it, the backend layer came out as nought tables and forty-four
// warnings that the migrations had invented their own schema.
//
...
async function findWorkbook(root) {
  ...
  for (const dir of WORKBOOK_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Opened here rather than in the caller, so the test is "is this the
      // schema workbook" and not "is it named like one". A file that will not
      // parse cannot be it either, which is why that failure is silent: it is
      // not an unreadable schema workbook, it is not a schema workbook.
      let sheets = null;
      try {
        sheets = readWorkbook(await readFile(path.join(root, rel)));
      } catch { /* not a readable workbook, so not this one */ }

      // The parse is carried rather than dropped and repeated — the winner is
      // the largest file in the delivery and would otherwise be read twice.
      if (sheets && WORKBOOK_SHEETS.every((name) => sheets.has(name))) {
        candidates.push({ rel, mtime: info.mtimeMs, size: info.size, sheets });
      } else {
        strangers.push({ rel, sheets: sheets ? [...sheets.keys()] : null });
      }
    }
  }
  candidates.sort((a, b) => b.mtime - a.mtime);
  return { candidates, strangers };
}
 ...
 */
/**
 * A cell holding several names as one string.
 *
 * The workbook writes these as `orders, ledger` in one column rather than as
 * several columns, so the split happens here and not at every reader. An empty
 * cell is an empty list rather than null: "nobody else writes this" and "the
 * question was not asked" both read as nothing downstream, and the first is
 * what a blank cell on a sheet that has the column actually means.
 */
...
export async function buildBackend(root, contractSchemas = []) {
  ...
  const { candidates: copies, strangers } = await findWorkbook(root);
  ...
  if (!workbookName) {
    problems.push({
      ...
      file: 'backend/',
      message: strangers.length
        ? `No workbook carrying a ${WORKBOOK_SHEETS.join(', ')} sheet — only the ADRs are shown. ` +
          `Passed over: ${strangers.map((other) => other.rel).join(', ')}`
        : 'No .xlsx schema reference in backend/ — only the ADRs are shown',
    });
    return { present: true, file: null, note: [], modules: [], tables: [], columns: {}, scaling: [], adrs, problems, stats: {} };
  }

  // ---- the workbook -------------------------------------------------------
  // Already parsed: finding it meant opening it, because its sheets are what
  // identified it. A file that would not read never became a candidate, so
  // there is no unreadable case left to handle here.
  ...
  // `Service` and `Foreign writers` arrived in the 25 August rebuild. The first
  // is what ships the table — the one thing the workbook could not say before —
  // and the second names who else may write it, which is the rule the service
  // boundary rests on: the owner defines the row, a foreign writer may append.
  const tableRows = table(sheet('Tables'), ['Module', 'Table', ['Columns', 'Cols']],
    ['Child of', 'Derived from', 'Migration', 'Written', 'New', 'PII',
      'Service', 'Foreign writers']).rows;
  ...
  const whereUsedRows = table(sheet('Where used'), ['Table'],
    ['Operations', 'Screens', 'Which operations', 'Which screens',
      'Owner service', 'Also written by']).rows
  ...
  const tables = tableRows
    ...
    .map((r) => {
      ...
      return {
        ...
        // What ships this table. Null on an older workbook, which is why every
        // reader of it has to cope with null rather than assume a service.
        service: r.Service || null,
        // Who else may write it. Held as a list because the interesting case is
        // the second name, not the first: one writer is a table inside its
        // service, two is a boundary somebody has to justify.
        foreignWriters: splitList(r['Foreign writers']),
        schemaId: schema?.id ?? null,
        schemaFile: schema?.file ?? null,
      };
    });

  // Where used states the owner a second time. Two independent statements of
  // one fact are worth more than either alone — but only if they are compared,
  // so a disagreement is reported and neither is quietly preferred.
  const ownerElsewhere = new Map(
    whereUsedRows
      .filter((r) => r['Owner service'])
      .map((r) => [r.Table, r['Owner service']])
  );
  for (const t of tables) {
    const other = ownerElsewhere.get(t.name);
    if (!t.service || !other || t.service === other) continue;
    problems.push({
      severity: 'warning',
      kind: 'backend-owner-disagreement',
      file: workbookName,
      message:
        `${t.name} is owned by ${t.service} on the Tables sheet and by ${other} ` +
        `on Where used. One of the two is stale.`,
    });
  }
