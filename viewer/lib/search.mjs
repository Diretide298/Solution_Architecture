...
export async function buildSearch(root, subjects = {}) {
  ...
  const pages = [
    ...
    // **Not a `.dc.html`, and that is the point of the name.** The two above are Claude Design
    // exports and this one is generated by `tools/build-readiness-page.py` out of the package's
    // own derived figures, so it carries the plain extension rather than borrowing a convention
    // that says where a file came from.
    handoffFiles.has('Build Readiness.html') && {
      id: 'build-readiness', name: 'Build readiness — can we start, and where',
      sub: 'the chain from requirement to table, counted link by link, and the nine open conflicts',
      pkgHref: '/handoff/Build Readiness.html',
      terms: 'build readiness start report coverage chain lineage provisional unagreed operations '
        + 'screens services boundaries cross service references row level security rls ddl '
        + 'permissions capabilities checklist conflicts open questions packs vertical slice '
        + 'sale and entry path where to start verdict gate checkers',
    },
