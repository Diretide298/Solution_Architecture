 ...
 */
const API_ROUTES =
  /^\/(api\/(auth|accounts|invites|reset|settings|links|work-packages|board|pms|changes|validation|verdicts|mentions|mentionable|alerts|scopes|ips|rates|costing|test-batches|agent|chat|health)(\/|$)|docs|openapi\.json)/;
...
async function refreshIndex(pkg, reason = 'startup') {
  ...
  pkg.indexing = (async () => {
    ...
    try {
      ...
      // How the package is built, shipped and run. Three folders nothing else
      // reads — `repos/`, `services/` and `deploy/` — and the findings are the
...
const WATCH_DIRS = [
  ...
  // What the delivery payload reads. `repos/` is deliberately not here: it is
  // mostly a generated mirror of the rest of the package, so watching it would
  // rebuild everything every time `derive-mirrors.py` runs — and the two files
  // per repo this layer actually reads are hand-authored and rarely touched.
  'deploy', 'services',
];
