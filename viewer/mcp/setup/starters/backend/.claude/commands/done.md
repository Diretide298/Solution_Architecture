...
1. Run `dotnet test TICVAI-Backend.slnx` and note the result.
2. Look at `git diff` and `git status` for what changed.
3. `adam_propose` ticket $ARGUMENTS: **Closed**, 100%, and a 2-4 line comment naming what was
   built and the test result. If the tests fail, propose **In progress** instead and say what fails.
