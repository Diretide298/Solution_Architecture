...
```
git clone <this repo> /opt/adam && cd /opt/adam/viewer
...
```
cd /opt/adam && git pull && cd viewer && sudo ./deploy/deploy.sh
```
...
`"root": "../ticvai"` means `/srv/ticvai/ticvai` on the server and `adam/ticvai`
