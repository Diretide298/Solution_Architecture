...
function serviceRow(svc, group, peak) {
  ...
  // **`max` is null on purpose and this used to render it.** The three deployed
  // services read "8-null replicas", because a range was drawn where the file
  // states a floor and then says why it states no ceiling: "a cap is a cap on
  // absorbing a peak nobody predicted, and the peak is the whole reason this
  // environment exists". A range with a null in it is the one reading that
  // makes a deliberate absence look like a missing value.
  const rep = el('div', `bu-svc-rep${svc.replicas ? '' : ' is-none'}`);
  if (!svc.replicas) {
    rep.textContent = 'not deployed';
  } else if (svc.replicas.max == null) {
    rep.append(el('span', 'bu-rep-n', `${svc.replicas.min}`));
    rep.append(el('span', 'bu-rep-k', svc.replicas.floorIsPeak ? 'at the peak' : 'floor'));
    rep.append(el('span', 'bu-rep-k', 'no ceiling'));
  } else {
    rep.append(el('span', 'bu-rep-n', `${svc.replicas.min}–${svc.replicas.max}`));
    rep.append(el('span', 'bu-rep-k', 'replicas'));
  }
  row.append(rep);
...
const RELATED = [
  {
    path: 'handoff/Burst Simulator.dc.html',
    href: '/handoff/Burst Simulator.dc.html',
    note: '**The same sale, solved rather than animated.** M/M/c per cluster, M/D/1 on the lease, eleven configurations run against one another — and it finds two the package has not: `catalogue.channel_capacity` refuses more than `inventory_hold` and is not flagged `contended`, and moving the reads out of the lock beats sharding at a fraction of the cost. Opens on its own; it reads nothing at runtime.',
  },
  {
    path: 'handoff/Shared Cell.dc.html',
    href: '/handoff/Shared Cell.dc.html',
    note: 'What runs between sales — the shared platform at rest, two venues to a cell. The other half of the picture the map above draws the burst half of.',
  },
  {
 ...
 *
 * **Shown only once the package answers for them.** They ship inside the drop
 * rather than with the viewer, so a package without them would otherwise offer
 * a chip that 404s. One HEAD each, and nothing appears until it returns.
 */
function revealHandoffPages() {
  const chips = [
    ['bu-sim-link', '/handoff/Burst Simulator.dc.html'],
    ['bu-cell-link', '/handoff/Shared Cell.dc.html'],
  ];
  for (const [id, href] of chips) {
    const node = $(id);
    if (!node) continue;
    const url = auth.apiUrl(href);
    fetch(url, { method: 'HEAD', credentials: 'include' })
      .then((res) => { if (res.ok) { node.href = url; node.hidden = false; } })
      .catch(() => { /* absent is the normal case for an older drop */ });
  }
}
...
/**
 * Everything the flash-sale view draws, into whatever host it is given.
 *
 * **Exported because this stopped being a page.** It was `burst.html`, with its
 * own header and its own chips; it is a tab of the CI/CD layer now, and the
 * only thing that had to change for that is which element it appends to. The
 * hero came from the page around it, so a caller that has none passes
 * `hero: true` and gets it drawn here instead.
 *
 * @param host  the element to fill
 * @param opts  `hero` — draw the title block, for a caller with no page header
 */
export async function renderBurst(host, { hero = false } = {}) {
  try {
    ...
    // A column for everything that is prose or a table, and the view itself
    ...
    if (hero) renderHero(column(), data);
    else if ($('bu-basis')) $('bu-basis').textContent = data.basis ?? '';
...
/** The title block, for a caller that is a tab rather than a page. */
function renderHero(host, data) {
  const sec = el('section', 'bu-hero');
  sec.append(el('p', 'bu-eyebrow', 'Deployment · scenario (c)'));
  sec.append(el('h1', 'bu-title', 'What a flash-sale environment actually runs'));
  const sub = el('p', 'bu-sub');
  sub.append(noteText(
    `Three services of sixteen, ${data.totals.operations} operations of `
    + `${fmt(data.totals.ofTotalOperations)}, ${data.totals.tables} tables of `
    + `${fmt(data.totals.ofTotalTables)}. The view is mostly about the other thirteen services: `
    + 'they are in the source with `deployed: false` rather than left out, because an '
    + 'environment defined by what it omits cannot be described by a list of what it keeps.',
  ));
  sec.append(sub);
  const src = el('p', 'bu-source');
  src.append(noteText(
    'Read live from **handoff/burst-scope.json** · derived by '
    + `**tools/derive-burst-scope.py** on every refresh · basis: **${data.basis ?? '—'}**`,
  ));
  sec.append(src);
  host.append(sec);
}
...
/**
 * The path a buyer actually makes, ranked.
 *
 * **The operations table below is sorted by this and still hides it.** Thirty-four
 * rows sorted by a column is a list you have to read; the top of the distribution
 * is a fact about the architecture — a handful of operations are most of every
 * request that arrives, and the ones at the top are reads. That is the same shape
 * the contended-table cards make for rows, and it was missing for the operations
 * that reach them.
 */
function renderHotPaths(host, data) {
  const ops = [...data.operations]
    .sort((a, b) => b.callsPerBuyer - a.callsPerBuyer || a.operationId.localeCompare(b.operationId));
  const total = ops.reduce((sum, o) => sum + o.callsPerBuyer, 0);
  if (!total) return;

  const top = ops.filter((o) => o.callsPerBuyer > 0).slice(0, 8);
  const share = top.reduce((sum, o) => sum + o.callsPerBuyer, 0) / total;
  const most = top[0]?.callsPerBuyer ?? 1;
  const contended = new Set(data.tables.filter((t) => t.contended).map((t) => t.table));

  const sec = el('section', 'bu-sec');
  sec.append(el('p', 'bu-h2', 'The hot path'));
  sec.append(el('h2', 'bu-h3', 'Most-called operations'));
  const lead = el('p', 'bu-lead');
  lead.append(noteText(
    `**${top.length} operations carry ${Math.round(share * 100)}% of every request a buyer makes.** `
    + `The other ${ops.length - top.length} share what is left. An environment sized against the `
    + 'average of thirty-four operations is sized against a number none of them is.',
  ));
  sec.append(lead);

  const list = el('div', 'bu-hot');
  for (const op of top) {
    const row = el('div', `bu-hot-row${op.lock ? ' is-contended' : ''}`);

    const head = el('div', 'bu-hot-head');
    head.append(el('span', 'bu-hot-id', op.operationId));
    head.append(el('span', `bu-verb ${op.verb.toLowerCase()}`, op.verb));
    head.append(el('span', 'bu-hot-path', op.path));
    row.append(head);

    const track = el('div', 'bu-hot-track');
    const fill = el('div', 'bu-hot-fill');
    fill.style.width = `${(op.callsPerBuyer / most) * 100}%`;
    track.append(fill);
    row.append(track);

    const foot = el('div', 'bu-hot-foot');
    foot.append(el('span', 'bu-hot-n',
      `${op.callsPerBuyer} ${op.callsPerBuyer === 1 ? 'call' : 'calls'} a buyer`));
    foot.append(el('span', 'bu-hot-pc',
      `${Math.round((op.callsPerBuyer / total) * 100)}% of the mix`));
    foot.append(el('span', 'bu-hot-svc', op.service.replace(/Service$/, '')));
    // What it touches is why a share matters: six calls a buyer to a read is a
    // cache question, three to a locked row is the sale failing.
    if (op.reads.length) foot.append(el('span', 'bu-hot-rw', `reads ${op.reads.length}`));
    if (op.writes.length) {
      foot.append(el('span', 'bu-hot-rw is-write', `writes ${op.writes.length}`));
    }
    if (op.lock) foot.append(el('span', 'bu-flag is-lock', op.lock));
    row.append(foot);

    // The tables underneath, named. The contended ones are the whole argument
    // of the section above this one, and this is where they are reached from.
    const touched = [...new Set([...op.reads, ...op.writes])];
    if (touched.length) {
      const tables = el('div', 'bu-hot-tables');
      for (const name of touched) {
        tables.append(el('span', `bu-hot-table${contended.has(name) ? ' is-contended' : ''}`, name));
      }
      row.append(tables);
    }
  ...
  const caveat = el('p', 'bu-lead');
  caveat.append(noteText(
    '`callsPerBuyer` is estimated and not measured — the caveat under the table below applies '
    + 'to the ranking exactly as much as to the numbers, because it *is* the ranking.',
  ));
  sec.append(caveat);
  host.append(sec);
}

// The page spelling, kept working. `burst.html` redirects into the layer now,
// so this only fires for a document that still carries the old markup.
if ($('burst-body')) {
  renderBurst($('burst-body')).finally(() => hideLoader());
}
