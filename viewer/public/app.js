...
// The account drawer and the bell. Their markup and behaviour live in one
// module so the standalone pages get the same chrome; this page supplies only
// the one thing that differs — where a mention lands.
import { mountAccountDrawer, mentionHash, closeAccountPanel, closeBellPanel }
  from './account-drawer.js';
// ── boot ─────────────────────────────────────────────────────────────
// What `mountAccountDrawer` handed back, for the few places that open or close
// the drawer from outside it.
let accountUI;
let graph;
let tree;
let er;
...
//
// **The drawer, the bell and their markup moved to `/account-drawer.js` on 21
// September** so the eight standalone pages can have them too. Nothing about
// the behaviour changed; what changed is that it is no longer only here. The
// one thing this page still decides is where a mention goes when it is
// clicked, because here there is something on screen to select.
function bindAccountUI() {
  accountUI = mountAccountDrawer({
    onTarget: (m) => { location.hash = mentionHash(m); },
  });
}
