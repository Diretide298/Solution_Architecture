...
function exportCsv(rows) {
  ...
  const body = [
    // `id` first, matching the service's export column for column. It is what
    // the admin panel's bulk close matches a row back to: (kind, artefact) has
    // a row per audience and another every time somebody changed their mind, so
    // a file without the id can only guess which verdict a decision belongs to.
    ['id',
