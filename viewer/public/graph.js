...
//
// These used to be the status tokens — 'info', 'ok', 'orange'. Every one of
// those is an alias of --accent, so five of the eight types below were drawn in
// the same teal and the legend claimed a distinction the picture did not make.
// --kind-* is the categorical set that exists to be told apart; see the note
// where it is defined.
const TYPE_TOKENS = {
  file: 'kind-contract',
  ...
  requestBody: 'kind-param',
  securityScheme: 'kind-security',
  permission: 'kind-permission',
};

// What a line between two nodes means. The indexer emits three: `contains` is
// "this contract defines that", `ref` is "$ref", `permission` is "this
// operation requires that scope". Drawing all three in the same neutral made
// the Local view a single teal mesh.
const EDGE_TOKENS = {
...
export function colorForNode(node, by = 'type') {
  // An enum is a schema with a closed set of values, and the difference matters
  // enough elsewhere in the app — the Schemas galaxy draws it — that it should
  // not vanish here.
  const type = node.type === 'schema' && node.enumValues ? 'enum' : node.type;
  const token = (by === 'group' ? GROUP_TOKENS[node.group] : null)
    ?? TYPE_TOKENS[type];
  return hue(token ?? 'text-faint');
}

/** The colour of a relationship, or null where the kind is not one we name. */
export function colorForLink(link) {
  const token = EDGE_TOKENS[link.kind];
  return token ? hue(token) : null;
}
...
export class Graph {
  ...
  draw() {
    ...
    for (const link of ordered) {
      ...
      // A caller's own colour wins — the Spine view sets one on purpose. Failing
      // that, the kind of relationship. Failing that, the old neutral.
      //
      // The two alphas are far apart for a reason: at rest a hundred lines have
      // to sit behind the nodes, and the moment one is picked out it has to
      // read as the thing being asked about.
      const tint = link.color ?? colorForLink(link);
      const stroke = active
        ? tint ? alpha(tint, .9) : alpha(accent, .75)
        : hasFocus
          ? (isLight ? 'rgba(0,0,0,.05)' : 'rgba(255,255,255,.035)')
          : tint ? alpha(tint, isLight ? .3 : .26)
          : (isLight ? 'rgba(0,0,0,.11)' : 'rgba(255,255,255,.09)');

      ctx.save();
      if (active && this.directed) {
        ctx.shadowColor = link.color ?? tint ?? alpha(accent, .6);
        ctx.shadowBlur = 10;
      }
      ctx.beginPath();
      ctx.moveTo(a.x, a.y);
