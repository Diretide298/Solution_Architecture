...
| **5** | **Is the conversational assistant available to guests, or staff only?** **Answered 17 August: yes, guests included, and AI is charged per token.** That resolves what we asked and raises what it costs you | **Two things follow, and one is yours to set.** 22.8.5 requires **live-agent handover**, which makes the concierge a support channel with a staffing model rather than a feature — somebody has to be on the other end. And **guest usage is bounded by footfall and curiosity, not by headcount**: a venue cannot stop guests asking questions, so an uncapped per-token charge is a bill you have no lever over. **We have built a cap into the metering and not set one.** What it should be, and whether it is a hard stop or a soft alert, is a commercial decision we would rather you made than we assumed |
