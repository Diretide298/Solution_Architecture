...
def main() -> int:
    ...
    for f in files:
        ...
        # **A device handover is not a navigation.** F29 step 4 moves from a server's handheld to a
        # wall-mounted kitchen display — the ticket appears there, nobody taps through. This check
        # refused it as a missing link and was right to refuse, because **a flow that crosses a
        # device boundary and calls it a link is a flow nobody has walked.**
        #
        # `crossesDevice: true` on the arriving step is the declaration, and it is deliberately
        # explicit. **Inferring it from platform codes would silently excuse every genuine missing
        # link between two platforms**, which is the defect this check exists to find.
        ordered = [(st.get("screen"), bool(st.get("crossesDevice")))
                   for st in steps if st.get("screen")]
        for (a, _), (b, handover) in zip(ordered, ordered[1:]):
