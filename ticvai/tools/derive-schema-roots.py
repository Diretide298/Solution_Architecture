...
def main() -> int:
    ...
    for sch, ts in by_schema.items():
        ...
        for t in sorted(ts):
            # **A declared child edge is the strong signal; a required foreign key on the first
            # column is the fallback.** Six tables carried a real parent link and no `child` edge
            # on 20 August — `ledger.journal_line` names `ledger.journal_entry` in its first
            # column and read as parentless, which made it look like a table with no key.
            parent = next((r["to_table"] for r in hard
                           if r["from_table"] == t and r["edge_kind"] == "child"), None)
            if not parent:
                first = (schema.get("cols") or {}).get(t) or []
                if first and first[0].get("references") and first[0]["references"] != t:
                    ref = first[0]["references"]
                    # only where the column is named for the table it points at — a line naming
                    # its entry is parentage; a line naming an account is a reference.
                    if first[0]["column"] == ref.split(".")[-1] + "_id":
                        parent = ref
            term = anchors.get(t, [])
            entry = {
                "schemaRoot": root,
