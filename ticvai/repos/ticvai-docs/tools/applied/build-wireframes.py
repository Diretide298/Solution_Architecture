...
def load_schemas() -> None:
    ...
    for sub in ("spine", "satellite", "shared"):
        for f in (CONTRACTS / sub).glob("*.yaml"):
            # **Windows defaults to cp1252 and this died on the first curly quote.** It is
            # the fourth tool found with this fault on 10 September, and all four had it for
            # the same reason: nothing runs them often enough for anyone to notice.
...
def main() -> int:
    ...
    for code, ss in screens.items():
        ...
        for s in ss:
            ...
            fn.write_text(screen_page(s, p, 2), encoding="utf-8")
            n += 1
            wf = (s.get("wireframe") or {}).get("status", "notStarted")
            cards.append(
        ...
        (OUT / f"{slug(code)}.html").write_text(page(p["name"], body, 0), encoding="utf-8")
    ...
    (OUT / "index.html").write_text(page("TICVAI wireframes", idx, 0), encoding="utf-8")
