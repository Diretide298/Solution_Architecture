...
`platform.workstation.department_id` both resolve to `platform.scope`. An outlet has a menu,
...
against `platform.scope` and none of them changes — an outlet appears on the path only where
