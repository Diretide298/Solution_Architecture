...
postgres instance  ·  region  ·  the cell
  ├── db: acme            the tenant template → tables → venue_id partitions
  ├── db: globex          the same template
  └── db: initech         the same template

services   shared across tenants, scaled on traffic,
           routed per request to a tenant database by tenant id
```

**The schemas stay as they are, inside each tenant database.** ADR-0028's decomposition is
...
**🔴 `control.cell` loses `tenant_id`.** **The contract had already half-moved on this** — the
field is nullable, and its own description says *"Null on a shared cell, which holds many. Nullable
since the ADR-0014 amendment of 14 August — it was required when a cell meant one tenant."* Under
this decision a cell never has one tenant, so a nullable column that is always null is not a
weaker version of the rule, it is the absence of one. A relation replaces it.

**🔴 Two places assert that CF-161 does not change the DDL, and both are now wrong.**
`Cell`'s description in `contracts/satellite/subscription.yaml`: *"CF-161 does not need a table
either. One database per cell or one per service is a build-time decision; the DDL is identical and
what changes is the connection string."* And `tools/derive-ddl.py`'s docstring: *"That blocks the
deployment topology and not the tables. The `CREATE TABLE` statements are identical either way."*
Both were right when written. This decision drops a column, adds three tables and takes `control`
out of the tenant template, so the DDL is not identical — and a claim that a question cannot affect
your artefact is the claim most likely to be left standing after it does.
...
**ADR-0032's pooling half is amended, and the number it needs does not exist.**
Pools are per database, so *"four hundred client connections share forty server ones"* is now a
statement about one tenant. **The figure to size against is concurrent tenants and the package has
never stated one.** It is a client question, and it should be asked before the instance is
specified rather than after.

**Answered 5 September: ten tenants at go-live, growing.** ADR-0032 now carries the arithmetic,
and the ceiling is **eleven tenants**, not the twenty-five estimated here — this division left out
`control`, which takes a pool of its own. **The configuration as it stands runs out one client
after launch**, and the lever is `default_pool_size` rather than `max_connections`.
...
**A schema per tenant inside one database.** Rejected. 26 schemas as of 3 September × 200
tenants is 5,200 schemas in
