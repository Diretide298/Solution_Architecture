#!/usr/bin/env python3
"""Attach the 19 September placement to the graph, the way 11 September was attached.

**798 screens were placed from 36 parsed packs and arrived with no navigation at all.**
`derive-board-flows.py` skips a board with *"no hub-and-spoke route in the navigation"*, so
without this every one of them is unreachable and no flow is derived for it.
...
**Why a sibling and not a re-run**, as on 11 September: an `applied/` tool records what was done on
a day, and re-running an older one over a different screen set promotes different hubs and writes
edges nobody decided.

**One change from the 11 September sibling.** That one named its four packs in a literal. This
selects **every pack that currently has a screen with no `entryFrom`**, because 36 packs is too
many to list by hand and a hand-written list is the thing that silently omits one. The selection is
self-limiting: a screen already wired is not in it, and the tool is idempotent regardless.
...
PROVENANCE = "structural — pack board {board} wiring, 19 September 2026"
...
def main() -> int:
    ...
    # every pack still holding an unwired screen - see the module docstring
    packs = {(s.get("source") or {}).get("pack")
             for s in screens.values()
             if (s.get("source") or {}).get("pack")
             and not ((s.get("navigation") or {}).get("entryFrom") or [])}
