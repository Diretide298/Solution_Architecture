...
`cross-region.yaml` 47 between them, every one on `x-ticvai-consumed-by`.
    ...
    **Keeps the first, not the last.** The first is the one written by hand with its original
    ...
    while i < len(lines):
        ...
        if m:
            ...
            # **List items may be indented differently in the two copies.** `safe_dump` writes
            # `- item` at the key's own indent; a hand-written block indents them further. Requiring
            # the two to match meant the check reported clean on `ai.yaml` while `check-package`
            # found four duplicates — **a de-duplicator that misses is worse than none**, because it
            # is trusted.
            item = r"^" + ind + r"\s*-\s"
            j = i + 1
            while j < len(lines) and re.match(item, lines[j]):
                j += 1
            if j < len(lines) and re.match(r"^" + ind + re.escape(key) + r":\s*$", lines[j]):
                k = j + 1
                while k < len(lines) and re.match(item, lines[k]):
                    k += 1
