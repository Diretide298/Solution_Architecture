# P10-overview-01 — P10 · Overview

**1 screens · 16 operations · 18 schemas · 10 permissions**
...
- **Every control that can be refused must be gated.** 10 permissions apply here:
  `CREDIT_MANAGE, CREDIT_OVERRIDE, ORDER_CREATE, ORDER_DISCOUNT, ORDER_EXCHANGE, ORDER_MODIFY, ORDER_REPRINT, ORDER_RESCHEDULE, ORDER_VIEW, ORDER_VOID`. A control nobody can use must say so,
...
|---|---|---|---|---|---|
| `PTR-002` | Partner Dashboard | listDetail | 16 | 2 | — |
...
[
 {
  ...
  "gaps": [
   {
    "operation": "getOrder",
    "why": "**3 declared operations reach no component on this screen**: getOrder, getOrderStatement, listOrderRefunds. Either the screen is missing what calls them, or the declaration is residue.",
  ...
  "overlays": [
   {
    ...
    "trigger": "Override",
    "body": "**Names what `overrideCreditLimit` changes and what it leaves alone**, in the consequence rather than the verb. A partner this affects should be identified in the dialog, not just counted.",
    "provenance": "contract orders.yaml POST /b2b-accounts/{accountId}/credit/override"
   },
   {
    "id": "confirmVoidOrder",
    "component": "confirmDialog",
    "trigger": "Void",
    "body": "**Names what `voidOrder` changes and what it leaves alone**, in the consequence rather than the verb. A partner this affects should be identified in the dialog, not just counted.",
    "provenance": "contract orders.yaml POST /orders/{orderId}/voids"
   }
  ],
  "states": {
   ...
   "emptyNoResults": "The filter narrowed it and the partner are still there. Names the active filter and offers to clear it.",
