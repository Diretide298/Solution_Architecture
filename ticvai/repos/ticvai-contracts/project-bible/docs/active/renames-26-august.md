...
| Was | Now | Why |
|---|---|---|
| `CrossRegionService` | **`CrossRegionService`** | Read quickly, `CrossRegion` is *cross-sell* — and this platform has a real cross-sell surface (`GST-058`, `createUpsellRule`). **Two things one letter apart, one about revenue and one about jurisdictions.** |
...
cross-region.RedemptionRight    -> cross-region.CrossRegionEntitlement
