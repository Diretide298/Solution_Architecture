...
-- reaches fnb.fnb_order through its keys; references fnb.delivery_location, platform.outlet
...
-- or listed Hangs off: reaches fnb.fnb_order through its keys; references fnb.delivery_location
...
-- sitting down Hangs off: reaches fnb.fnb_order through its keys; references fnb.table_visit,
...
-- one thing an inspector looks for. Hangs off: reaches fnb.fnb_order through its keys; references
-- fnb.corrective_action, identity.principal, venuemap.point. Reached by: 2 operations read it and
-- 1 write it
