...
-- convention, not declared: control.api_quota.client_id -> control.api_client
CREATE INDEX IF NOT EXISTS ix_api_quota_client_id ON control.api_quota (client_id);
...
-- convention, not declared: control.onboarding_application.requested_plan_id -> control.subscription_plan
...
-- convention, not declared: control.partner_agreement.branding_asset_id -> maintenance.asset
...
-- crosses the database boundary: control.cell.region_id -> platform.org_unit
CREATE INDEX IF NOT EXISTS ix_cell_region_id ON control.cell (region_id);
-- crosses the database boundary: control.cell_cluster.region_id -> platform.org_unit
...
-- crosses the database boundary: control.subscription.tenant_id -> platform.tenant
CREATE INDEX IF NOT EXISTS ix_subscription_tenant_id ON control.subscription (tenant_id);
...
-- crosses the database boundary: control.usage_record.venue_id -> platform.org_unit
