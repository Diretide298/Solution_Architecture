# Handoff: Adam — viewer UI redesign

## Screens in this package
The **landing page** — five layer nodes in 3D space, scrolled through and clicked into — has its own
spec in `LANDING.md`, and the contract graph's rotating sphere in `GRAPH.md`. Both carry a
`3D / 2D` toggle. The viewer shell (day + night) is specified below. The **invite** and **sign-in** pages (day + night each) have
...
Brand: the product is **Adam — the context layer for AI-assisted development**. The header carries
the Adam lockup (mark + wordmark) as a single image; the foot of the left column carries a
...
- `Adam Landing.dc.html` — the landing page (spec in `LANDING.md`)
...
- Brand: the Adam lockup as one image, `height:46px; width:auto`. Two files, one per theme —
  `brand/adam-lockup-day.png` on the day chrome, `brand/adam-lockup-night.png` on the night
...
and a **Fit** button. Below it a 520px panel, `radius:15px`, filled with a deep radial field
(`radial-gradient(120% 92% at 50% 46%, #16262C, #111A1F 38%, #0D1113 72%, #0A0C0E 100%)`) and a
single `<canvas>` at `inset:0`. **Both themes use the same dark field** — the glow the view is
built on does not survive a light background.

The diagram is a **slowly rotating 3D sphere**, drawn on canvas rather than in the DOM.
Full spec — geometry, projection, per-frame draw order and every constant — is in `GRAPH.md`;
the executable reference is `buildGalaxy()` / `drawGalaxy()` in either design file.

In short: 24 labelled contract hubs on a Fibonacci sphere (the two shared contracts on an inner
0.34 shell), ~900 unlabelled motes on the surface tied to their three nearest neighbours by
filaments, and the 22 event links drawn hub-to-hub with a packet travelling publisher → consumer.
Depth drives size, brightness and label opacity; labels are drawn only on the near face. Amber is
a link with a critical consumer and the shared core; teal is everything else.

A legend strip is docked across the foot of the panel on a bottom-up gradient scrim — dots at 8px,
labels 11.5px, closing with "dot size is the operation count · the sphere turns on its own".
...
## Assets
`assets/brand/` — the Adam identity.
- `adam-lockup-day.png` / `adam-lockup-night.png` — the header lockups, transparent background.
  These are the two files the viewer actually loads.
- `adam-mark.png` — mark only, transparent, for favicons, avatars and tight spaces.
- `adam-logo-light.svg` / `adam-logo-night.svg` — a redrawn vector lockup (mark as geometry,
...
- `adam-mark-day.svg` / `adam-mark-night.svg` — the vector mark alone.
...
tokens above are enough to restyle them. The canvas renderers (`boxdiagram.js`, `statemachine.js`) should read their palette from the CSS
custom properties rather than literals, so the day/night toggle reaches them too. `graph.js` is the
exception: the Spine scope is replaced outright by the sphere in `GRAPH.md`, which owns its own
palette because it sits on a fixed dark field in both themes.
