...
- `designs/Adam Invite.dc.html` — night
- `designs/Adam Invite Day.dc.html` — day
...
## Brand
Night uses `brand/adam-lockup-night.png` at 44px tall. Day cannot — that lockup's wordmark is
white — so it composes the mark and the wordmark: `brand/adam-mark-day.png` at 44px, a 1px
rule, then `Adam` in **Montserrat Light**, 22px, letter-spacing .3em, #16323A. If a dark-wordmark
lockup gets produced later, swap it in and drop the composed version.


# Sign-in page

- `designs/Adam Sign In.dc.html` — night
- `designs/Adam Sign In Day.dc.html` — day
...
`@keyframes adam-caret` at `1s steps(1) infinite`.

**Findings** rise in with `@keyframes adam-rise` (.35s, 6px). Two tones: **ok** teal, **warn** amber.
