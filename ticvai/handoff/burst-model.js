...
export const VARIANTS = [
  {
    ...
    attacks: '—',
    inPackage: true,
    note: 'The environment as written. Three services, 17 tables, 45 containers, and gone afterwards.',
    params: {},
  },
  {
    key: 'B',
  ...
  },
  {
    ...
    attacks: 'the wall',
    inPackage: true,
    note: 'The only variant that attacks the actual wall. A buyer who does not care which seat takes any shard with capacity, so sixteen proceed where one did — but a buyer choosing a specific seat still serialises, because they are contending for a seat rather than for capacity. The cost is reconciliation: sixteen sub-pools each empty separately, so fifteen empty and one holding four is a performance that looks sold out to most buyers. Rebalancing is the hard part and the variant does not implement it.',
  ...
  {
    key: 'E',
    name: 'E · shrink the lease',
    file: 'not in the package — a code change, not an architecture change',
    changes: 'the idempotency check and both reads move outside the lock',
    attacks: 'the wall, cheaply',
    inPackage: false,
    note: 'Do the cheap thing first. cache:idempotency is checked before the transaction opens; platform.workstation is the holder, resolvable from the session, and has nothing to do with capacity; channel_capacity is read first and the lock taken only to decrement. If the lease drops from 6ms to 0.5ms the per-row ceiling goes from 167/s to 2,000/s — more than sixteen shards would give, with no fragmentation and no reconciliation obligation.',
    params: { lockMs: 0.5 },
  },
  {
    key: 'F',
...
/** The lease hold in force — a variant may shorten it or remove the lock. */
export const lockMsFor = (cfg) => {
  const p = (VARIANTS.find((v) => v.key === (cfg?.variant ?? 'A')) ?? VARIANTS[0]).params;
  return p.lockMs ?? cfg?.lockMs ?? 6;
};
...
export const DEFAULTS = {
  ...
  lockMs: 6,              // mine — lease hold time on a contended row
...
export const wantedFor = (cluster, lambda, threshold) => Math.max(
  cluster.min,
