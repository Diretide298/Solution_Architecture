...
|---|---:|
| `platform.scope_node` | 289 |
| `identity.role` | 207 |
| `ledger.legal_entity` | 75 |
| `pii.subject` | 62 |
| `access.admission_profile` | 57 |
| `control.plan` | 25 |
| `catalogue.variant` | 21 |
| `ai.knowledge_collection` | 18 |
| `control.developer_account` | 8 |
| `whitelabel.content_page` | 7 |
...
- **Reaches the root through nothing** — `bill_split`, `delivery_location`, `delivery_location_outlet`, `kitchen_station`, `kitchen_ticket`, `kitchen_ticket_line`, `location_code`, `location_session`, `menu`, `menu_item`, `menu_section`, `modifier_group`, `modifier_option`, `production_run`, `recipe`, `recipe_ingredient`, `sub_bill`, `table`, `table_reservation`, `table_session`, `table_visit`, `waitlist_entry`
...
- **Reaches the root through nothing** — `audit_read`, `denomination`, `dsar_request`, `guest_link`, `outbox`, `schema_version`, `wallet_authorisation`
...
**Root: `inventory.item`**  ·  own 8 · reach 10 · out 2

- **1 step from the root** — `goods_receipt_line`, `movement`, `purchase_order_line`, `quotation_line`, `requisition_line`, `stock_batch`, `stock_level`, `transfer_line`
...
- **Reaches the root through nothing** — `channel_allocation`, `donation_campaign`, `entitlement_template`, `envelope`, `event`, `import_job`, `inventory_lease`, `performance`, `price_list`, `published_bundle`

  Standalone configuration, or a table whose foreign key is not declared. **Not a defect on its own** — a password policy belongs to a scope rather than to a principal — but it is where an undeclared key hides.

## `identity` — 16 tables

**Root: `identity.principal`**  ·  own 11 · reach 111 · out 2

- **1 step from the root** — `authz_audit`, `grant`, `mfa_challenge`, `mfa_method`, `mfa_recovery_code`, `principal_credential`, `session`

- **Reaches the root through nothing** — `guest_session`, `otp_challenge`, `password_policy`, `role`, `role_permission`, `segregation_rule`, `sso_group_mapping`, `sso_provider`
...
- **Reaches the root through nothing** — `chunk_ref`, `conversation`, `interaction`, `knowledge_collection`, `knowledge_document`, `layout_draft`, `message`, `policy`, `proposed_action`, `provider`
...
**Root: `games.prize`**  ·  own 1 · reach 1 · out 2

- **1 step from the root** — `redemption_line`

- **Reaches the root through nothing** — `card`, `credit_ledger`, `game`, `play`, `reader_profile`, `redemption`

  Standalone configuration, or a table whose foreign key is not declared. **Not a defect on its own** — a password policy belongs to a scope rather than to a principal — but it is where an undeclared key hides.

## `maintenance` — 8 tables

**Root: `maintenance.asset`**  ·  own 4 · reach 12 · out 3
...
**Root: `venuemap.point`**  ·  own 3 · reach 4 · out 4
